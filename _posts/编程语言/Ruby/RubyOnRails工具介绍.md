...
定义关联关系之后，写代码变简单了，例如定义作者和书一对多关系，那么删除作者时候作者写的书也会被自动删除掉。

如果在表A中加入如下字段：

| 字段 | 说明 | 实际作用 | 举例 |
|:---:|:---:|:---:|:---:|
| belong_to B | 强调一个A只属于一个B，注意其中暗含的意思是A的抽象范围比B要小 | 作用是在A里面加入B的id字段 | 在一个特定时间点：一个球员（例如梅西、哈维）只能属于一支球队（巴萨），球员信息里面加入所属球队的名字 |
| has_one B | 强调一个A只有一个B，注意其中暗含的意思是A的抽象范围比B要大 | 作用是在B里面加入A的id | 在一个特定时间点：一个俱乐部有多个球员，不符合只有一个的要求，因此不可以使用has_one；一支球队（国米、A米）只有一个主场（圣西罗，也叫梅阿查） |
| has_many B | 有多个B，且是一对多 | |
