...
## 工作会话流程（每次切换到本 agent 时执行）

1. **开场先看 TODO Tasks**：从未完成的最高优先级任务继续，或按用户指定的任务执行。
2. **改动前先本地验证**：涉及布局/样式的改动，提醒用户用 `bundle exec jekyll serve` 预览确认后再推送。
3. **完成任务后立即更新 TODO 状态**：`[ ]` → `[x]`，附一句实现要点（方案、关键文件）。
4. **提交规范**：中文 commit message，说清楚改了什么、为什么；推送前向用户确认。

原则：博客是生产环境（www.micili.cn），宁可多验证一步，不要破坏线上展示。
...
- [ ] 中文排版检查
  - 自动检测中英文之间缺少空格、全角半角混用等
- [ ] Front Matter 校验
  - 检查每篇文章的 front matter 字段是否完整
- [ ] 自动合并低风险依赖更新
  - 配合 Dependabot，patch 级别更新自动合并
...
- [x] 深色模式（跟随系统 + 手动切换，localStorage 记忆）
- [x] 返回顶部按钮（内联方案，所有设备通用）
- [x] 预计阅读时间（Liquid 计算字数，文章头部显示）
- [ ] 全站搜索 — 用 lunr.js 纯前端实现，无需服务器
- [ ] 文章分享按钮 — 微信/微博/Twitter
- [ ] 相关文章推荐 — 文章底部根据标签推荐类似文章
- [ ] 结构化数据（JSON-LD）— Google 搜索结果显示更丰富信息
- [ ] Open Graph 图片自动生成 — 分享链接时自动生成带标题的封面图
- [ ] Leetcode 进度面板 — 首页或侧栏展示刷题进度
- [ ] 圣经阅读进度 — 展示各卷书的阅读/笔记完成度
